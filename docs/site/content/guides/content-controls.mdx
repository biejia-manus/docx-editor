...
Toggling a checkbox in a table preserves its enclosing cell, row, and formatting,
including after you save and reopen the document. The toggle rewrites only the run that
shows the state glyph. If the control's content holds no paragraph and no run the editor
can identify, such as a nested control or foreign markup, the write is refused instead of
...
Use `popups.contentControl` for the control popup and `popups.contentControlWidget`
for dropdown, combo box, and date entry. Omitted entries retain their default UI.
See [Customize popups](/docs/2.x/guides/customize-dialogs) for renderer ownership and sessions.
