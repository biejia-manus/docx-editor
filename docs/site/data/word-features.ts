...
export const wordFeatures: WordFeature[] = [
  ...
  },
  {
    ...
    tier: 'community',
    notes:
      'Protects graphemes, punctuation, and full-width number groups across run boundaries. Document theme languages independently select supplemental Latin, East Asian, and complex-script faces. A resolved body Latin theme face supplies the default when inherited and direct Latin font references are absent. A resolved body East Asian theme face supplies missing East Asian defaults after style inheritance. Without that setting, empty East Asian theme faces use inherited Chinese, Japanese, or Korean language hints, with named CJK defaults when no face resolves. Live and export font resolvers receive the selected faces. East Asian font hints cover supported punctuation, symbols, Greek, and Cyrillic ranges while preserving explicit symbol fonts. Reads kinsoku, wordWrap, overflowPunct, strictFirstAndLastChars, language-specific custom line-break sets, and characterSpacingControl. Korean character wrapping follows wordWrap. Compression removes one shared side bearing between adjacent punctuation. Paragraph and explicit line-break boundaries retain their advances. Colons can compress before opening brackets, bounded by the following bracket bearing. Measured colon/closing-punctuation pairs can halve the colon advance, including authored character spacing. If wrapping separates the pair, the paragraph retains natural colon advances. Highlighted and shaded colons retain their advances. Opening glyphs carry an explicit ink offset that native text selection preserves. Authored spaces, other centered punctuation, outlined text, decorated text, and tracked changes retain their advances. Mirrored RTL punctuation retains its full advance. Kana uses deterministic advance reductions. Narrow plain LTR East Asian paragraphs can use measured punctuation bearings to fit an ordinary glyph and its protected punctuation group. Mixed Latin text, fields, decorations, revisions, RTL text, unavailable ink bounds, and float passages retain conservative fitting. Vertical Japanese composition is not modeled. Typography settings have no dedicated UI.',
  },
  {
